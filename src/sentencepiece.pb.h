...
public:
  ...
  public:
    ...
    void set_piece(const std::string& value) { piece_ = value; }
    void set_piece(std::string&& value) { piece_ = value; }
    void set_piece(const char* value) { piece_ = value; }
    void set_piece(const char* value, size_t size) { piece_ = std::string(value, size); };

    const uint32_t& id() const { return id_; }
    void set_id(uint32_t value) { id_ = value; }

    const std::string& surface() const { return surface_; }
    void set_surface(const std::string& value) { surface_ = value; }
    void set_surface(std::string&& value) { surface_ = value; }
    void set_surface(const char* value) { surface_ = value; }
    void set_surface(const char* value, size_t size) { surface_ = std::string(value, size); };

    const uint32_t& begin() const { return begin_; }
    void set_begin(uint32_t value) { begin_ = value; }

    const uint32_t& end() const { return end_; }
    void set_end(uint32_t value) { end_ = value; }
  ...
  const Vector<SentencePiece>& pieces() const { return pieces_; }
  SentencePiece* add_pieces() { return pieces_.Add(); }
  SentencePiece* mutable_pieces(int index) { return &pieces_[index]; }
  Vector<SentencePiece>* mutable_pieces() { return &pieces_; }
  int pieces_size() const { return pieces_.size(); }

  const std::string& text() const { return text_; }
  void set_text(const std::string& value) { text_ = value; }
  void set_text(std::string&& value) { text_ = value; }
  void set_text(const char* value) { text_ = value; }
  void set_text(const char* value, size_t size) { text_ = std::string(value, size); };

  std::string* mutable_text() { return &text_; }

  const float& score() const { return score_; }
  void set_score(float value) { score_ = value; }

  void Clear() { text_.clear(); pieces_.clear(); score_ = 0; }

  std::string Utf8DebugString() { return text_; }

private:
  std::string text_;
  Vector<SentencePiece> pieces_;
  float score_;
};

class NBestSentencePieceText {
public:
  const Vector<SentencePieceText>& nbests() const { return nbests_; }
  SentencePieceText* add_nbests() { return nbests_.Add(); }

  std::string Utf8DebugString() { return "Dummy"; }
  void Clear() { nbests_.clear(); }

private:
  Vector<SentencePieceText> nbests_;
};
